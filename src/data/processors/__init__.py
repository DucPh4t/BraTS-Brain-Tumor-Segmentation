"""
Mô tả: Bộ khởi tạo Factory để trả về hàm tiền xử lý và chuẩn hóa ảnh MRI tương ứng dựa vào cấu hình cấu hình (min_max, zscore_clip, zscore_clip_custom,...).
Đầu vào:
    norm_type (str): Tên phương thức chuẩn hóa.
    config (dict): Cấu hình bổ sung.
Đầu ra:
    Trả về hàm tiền xử lý (preprocessor function) cho ảnh MRI.
"""
...
def get_preprocessor(norm_type, config=None):
    """Hàm Factory trả về hàm tiền xử lý tương ứng."""
    config = config or {}
    if norm_type == "min_max":
        ...
        # Trường hợp mặc định: Không chuẩn hóa
