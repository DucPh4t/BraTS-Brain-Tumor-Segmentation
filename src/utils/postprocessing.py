"""
Mô tả: Bộ tiền xử lý hậu kỳ (Post-processing) lọc nhiễu vùng u nhỏ sử dụng thuật toán Connected Components để cải thiện chỉ số phân đoạn (đặc biệt loại bỏ các đốm nhiễu False Positive).
Đầu vào:
    mask (np.ndarray): Mặt nạ dự đoán nhị phân.
    min_voxels (int): Ngưỡng thể tích tối thiểu (theo voxel) để giữ lại.
    keep_largest (bool): Chỉ giữ lại vùng kết nối lớn nhất.
Đầu ra:
    Trả về mặt nạ nhị phân đã lọc sạch nhiễu.
"""
...
def enforce_region_hierarchy(pred_volume: np.ndarray) -> np.ndarray:
    """Đảm bảo tính hợp lệ của phân cấp vùng u BraTS: ET phải nằm trong TC, TC phải nằm trong WT."""
