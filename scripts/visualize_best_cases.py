"""
Mô tả: Chạy dự đoán và tạo ảnh động GIF mô phỏng quét lát cắt cho các ca phân đoạn tốt nhất (như Training 347, 152, 189...).
Đầu vào:
...
def main():
    ...
    out_dir = "outputs/figures/best_5_cases"
    ...
    for sid in BEST_CASES:
        ...
        sid_out_dir = os.path.join(out_dir, f"{sid}_all_slices")
        os.makedirs(sid_out_dir, exist_ok=True)
        
        frames = []
        for s_idx in tqdm(range(155), desc=f"Generating frames for {sid}"):
            frame = get_slice_frame(s_idx, vols, preds, sid)
            # Save single image
            out_path = os.path.join(sid_out_dir, f"slice_{s_idx:03d}.png")
